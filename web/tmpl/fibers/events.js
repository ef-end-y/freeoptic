...
cy.on('zoom pan', function(event)
{
	...
	const change_scheme_detailization_zoom = fibers.settings.change_scheme_detailization_zoom * (0.35 + units_count * 0.65/1000);
	const detailization = zoom > change_scheme_detailization_zoom;
	
	if( (event.type == 'zoom') && (detailization && simple) || (!detailization && !simple) )
	{
